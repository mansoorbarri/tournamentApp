...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof eventTypeID !== 'number' || typeof description !== 'string') {
      return NextResponse.json({ message: 'Invalid data types' }, { status: 400 });
    }

    const newEventType = new EventType({
      eventTypeID: Number(eventTypeID), // Explicitly cast to number
      description,
    });
    ...
    return NextResponse.json(
      { message: 'Event type added', data: savedEventType },
      { status: 201 }
    );
  } catch (error) {
    console.error('Error adding event type:', error);
    return NextResponse.json(
      { message: 'Error adding event type', error },
      { status: 500 }
    );
  }
}
